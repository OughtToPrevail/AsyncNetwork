...
public class WindowsSelectorImpl implements WindowsSelector
{
	...
	public WindowsSelectorImpl() throws LoadException
	{
		if(!implemented)
		{
			NativeLoader.exception("WindowsSelectorImpl");
		}
	}
	...
	private final IndexedList<PendingWrite> pendingWrites = new IndexedListImpl<>();
	...
	{
		int index = select0(handle, timeout, result);
		if(index == -1)
		{
			return null;
		}
